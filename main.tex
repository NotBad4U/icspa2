...
\begin{frame}[fragile]{Alethe SMT proof}
    \begin{minted}[allowlocal,autogobble, fontsize=\footnotesize, frame=single]{smt.py:CustomLexer}
        (assume a0 (p a))
        (assume a1 (= a b))
        (assume a2 (not (p b)))
...
\begin{definition}[Alethe step notation]
...
\end{definition}
\vfill
\begin{definition}[Definition context]
...
\begin{lemma}[Clause equivalence with disjunctions]
...
&= ~cong2_{f}(f~p_1 \dots cong2_f(f~p_{n-1}~p_n))[ \Gamma \vdash i: \mathcal{F}(f~ t_1 \dots t_n \approx f~ u_1 u_n) ] && \\
& \text{otherwise}~f\_equal_n(f~p_1~\dots~p_n)[ \Gamma \vdash i: \mathcal{F}(f~ t_1 \dots t_n \approx f~ u_1 u_n) ], && \\
